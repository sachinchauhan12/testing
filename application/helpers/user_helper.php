...
// This function used for get name by user_id========
	...
	*/
    ...
    function RoleAccess($pageName, $role){
       if($role=='1'){
		   ## for admin access here
            $rightsArr=array('dashboard','registeruser','registerlist','category','subcategory','Manageproduct','productlicense','productlocation','producttype','product','coupon','order','blog','job','service','page','banner','client','homeCtrl');
       }
       if($role=='2'){
            $rightsArr=array('dashboard','category','subcategory','Manageproduct','productlicense','productlocation','producttype','product','coupon','order','blog','job','service','page','homeCtrl');
       }
       if($role=='3'){
		    ## for Finance user access here
            $rightsArr=array('dashboard','order');
       }
       if($role=='4'){
		    ## for Support user access here
			$rightsArr=array('dashboard','page','job');
        }
        if($role=='5'){ 
			## for Content user access here
            $rightsArr=array('dashboard','service','Manageproduct','productlicense','productlocation','producttype','product','blog','service');
       }
       if(in_array($pageName, $rightsArr)){
          return true;
       }else{
          return false;
       }
