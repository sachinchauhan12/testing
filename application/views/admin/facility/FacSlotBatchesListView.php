				...
				<div class="main-content-inner">
					...
					<div class="page-content">
						   <div class="row">
									...
									<label class="col-md-12 control-label no-padding-right" for="form-field-1">Type </label>

									<select name="facility_type" id="facility_types"  class="form_input">
									<option value="">Type</option>
									<option value="academy"<?php if(@$facility_type == "academy") echo "selected"; ?>>Academy</option>
									<option value="facility"<?php if(@$facility_type == "facility") echo "selected"; ?>>Facility</option>
									</select>
									...
									<?php echo form_submit(['class'=>'btn btn-info ordersmall','name'=>'Submit','id'=>'slot_submit','value'=>'Submit']); ?> 
									<a href="<?=base_url('admin/facility/facilityslotbatchesList');?>" class="btn btn-info clearbtn" id="clearbtn">
										...
										<div class="table-header">
										 Slot / Batch List
											...
											<form method="post" action="<?=base_url('admin/facility/exportdatabyslot');?>" id="excel_export">
											  ...
											  <input type="hidden" name="facility_types" id="facility_typess" value="">
											...
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													...
													<tr>
														...
														<td><?=$facility_batch_data->user_name;?></td>
														<td><?=$facility_batch_data->fac_name;?></td>
														<td><?=ucfirst($facility_batch_data->fac_type);?></td>
														...
														<td>
														<?=ucfirst($facility_batch_data->fac_batch_slot_status);?>
														<!--<select name="facslot" onchange="changebatchstatus('<?php echo $facility_batch_data->batch_slot_id; ?>')" class="facslotbatchstatuss<?php echo $facility_batch_data->batch_slot_id;?>">
...
var batch_slot_status =  $('.facslotbatchstatuss'+batch_slot_id+' option:selected').val();
			  // alert(batch_slot_status);
			   // return false;
			   $.ajax({
				   type: 'post',
...
$(document).ready(function(){		
	$(document).on('click','#slot_submit',function(){
		...
		var facilty_slot_data=document.querySelector('#facilty_slotydata');
		 facilty_slot_data.addEventListener('click',function(){
			 ...
			 var facility_types= document.getElementById('facility_types').value; 
			 // var facility_type= document.getElementsByClassName('facility_types').value; 
		  ...
		  document.getElementById('facility_typess').value=facility_types; 
		  $('#excel_export').submit();		
		 });
